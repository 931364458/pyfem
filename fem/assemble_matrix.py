...
def assemble_cells(data, kernel, dofmap: DofMapWrapper, mesh: MeshWrapper, coeffs, constants, perm, active_cells):
    ...
    for idx in active_cells:
        ...
        data[idx * local_mat.size: idx * local_mat.size +
             local_mat.size] += local_mat.ravel()
...
def sparsity_pattern(dofmap: DofMapWrapper):
    ...
    cols = numpy.tile(numpy.reshape(dofmap.dof_array,
                                    (num_cells, dofmap.num_cell_dofs)), dofmap.num_cell_dofs)
